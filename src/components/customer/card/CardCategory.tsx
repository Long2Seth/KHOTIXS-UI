    ...
    return (
        <>
            {isLoading ? <SkeletonCardCategoryComponent /> :
                <section className="container mx-auto w-full bg-khotixs-background-white dark:bg-khotixs-background-dark font-bold flex flex-col justify-center items-center h-auto pt-[50px]">
                    ...
                    <section className=" justify-center gap-2 flex flex-wrap rounded-[6px] max-w-[400px] md:max-w-[500px] lg:max-w-[700px] xl:max-w-[900px] 2xl:max-w-[1200px] sm:w-full sm:grid sm:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-6 md:gap-5 ">
                        {categories.map((category: CategoriesIcon) => (
                            <a
                                key={category.name}
                                href={category.link}
                                className=" dark:border-white border-2 dark:hover:bg-primary-color rounded-[15px] w-[145px] h-[50px] sm:w-full sm:h-[65px] md:h-[80px] lg:h-[120px] flex flex-col justify-center items-center shadow-lg">
