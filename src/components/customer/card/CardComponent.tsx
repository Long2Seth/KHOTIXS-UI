...
// Let's also log the full result of the map operation for comparison
    ...
    return (
        <section
            ...
            className="relative cursor-pointer bg-white dark:bg-backdrop-blur dark:bg-opacity-5 rounded-[10px] flex flex-col justify-start items-start max-w-[300px] sm:max-w-[300px] md:max-w-[330px] md:h-[340px] xl:max-w-[400px] xl:h-[450px] 2xl:h-[480px] h-[280px] sm:h-[250px] 2xl:max-w-[450px] md:p-0">
            ...
            <a className="group block overflow-hidden rounded-[10px]">
                <div className="rounded-tr-[10px] z-10 object-cover rounded-tl-[10px] w-full h-[58%] 2xl:h-[50%] md:h-[50%] overflow-hidden">
                    <Image
                        ...
                        src={`${event.image}`}
                        alt=""
                        className="z-10 object-cover rounded-tr-[10px] rounded-tl-[10px] w-full bg-cover bg-center transform transition-transform duration-300 group-hover:scale-110"
                    />
                </div>
                <section
                ...
                <div className="absolute bottom-2 right-1">
                    {
                        event.tickets
                            .map((ticket, index) => (
                                <Badge
                                    key={index}
                                    className={`text-white rounded-[6px] mx-0.5 ${getLabelClass(ticket.type)}`}>
