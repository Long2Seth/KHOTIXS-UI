...
export function SkeletonCardCategoryComponent() {
    return (
        <section className="container mx-auto w-full py-12 font-bold flex flex-col justify-center items-center h-auto">
            {/* Title Skeleton */}
            ...
            <section className="px-5 mt-2 justify-center gap-3 flex flex-wrap sm:w-full sm:grid sm:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-6">
                {Array.from({ length: 6 }).map((_, index) => (
                    <div
                        key={index}
                        className=" bg-secondary-color-text dark:border  dark:bg-khotixs-background-dark rounded-lg w-[145px] h-[65px] sm:w-full sm:h-[85px] md:h-[100px] lg:h-[140px] flex flex-col justify-center items-center shadow-lg"
                    >
                        {/* Icon Skeleton */}
